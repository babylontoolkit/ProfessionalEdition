...
public class AI_CodeAssistant : EditorWindow
{
    ...
    public const string CODEWRX_AI_EXTENSION = "https://github.com/codewrxai/ai-code-assistant-plugin";
    ...
    [MenuItem("Tools/" + CanvasToolsStatics.CANVAS_TOOLS_MENU + "/Developer Options/AI Code Assistant Plugin", false, 87)]
    ...
    public static async Task<string> PostTrainingFileAsync(string endpoint, string apikey, string filePath)
    {
    ...
    public static async Task<string> PostSafeWebRequestAsync(string endpoint, string apikey, string payload)
    {
        using (HttpClient httpClient = new HttpClient())
        {
            // set max timeout
    ...
    public static async Task<string> GetSafeWebRequestAsync(string endpoint, string apikey)
    {
        using (HttpClient httpClient = new HttpClient())
        {
            // set max timeout
    ...
    public static async Task<string> DeleteWebRequestAsync(string endpoint, string apikey, string model)
    {
    ...
    public void OnInitialize()
    {
        ...
        float dialogHeight = (this.logicModel == OpenAILogicModel.gptX) ? 382.0f : 306.0f;
    ...
    public void OnGUI()
    {
        ...
        logicModel = (OpenAILogicModel)EditorGUILayout.EnumPopup("AI Logic Model:", (OpenAILogicModel)logicModel, GUILayout.ExpandWidth(true));
        EditorGUILayout.Space();
        ...
        EditorGUI.BeginDisabledGroup(logicModel == OpenAILogicModel.gptX && this.selectMode == true);
            if (logicModel == OpenAILogicModel.gptX)
            {
                sourceControl = (OpenAISourceControl)EditorGUILayout.EnumPopup("Source Control:", (OpenAISourceControl)sourceControl, GUILayout.ExpandWidth(true));
                EditorGUILayout.Space();
            }
            ...
            maxTokens = (int)EditorGUILayout.Slider("Completion Tokens:", maxTokens, AI_CodeAssistant.MINIMUM_ALLOWED_TOKENS, AI_CodeAssistant.MAXIMUM_ALLOWED_TOKENS);
            EditorGUILayout.Space();

            if (showCredentials == true)
            {
                openAIAPIKey = EditorGUILayout.TextField("Secret API Key:", openAIAPIKey);
                EditorGUILayout.Space();
            }
            else
            {
                openAIAPIKey = EditorGUILayout.PasswordField("Secret API Key:", openAIAPIKey);
                EditorGUILayout.Space();
            }

            showCredentials = EditorGUILayout.Toggle("Show Credentials:", showCredentials);
            EditorGUILayout.Space();

            EditorGUILayout.BeginHorizontal();
            if (GUILayout.Button("Reset Code Assistant Settings"))
            {
                if (UnityTools.ShowMessage("Are you sure want to reset the settings?", CanvasToolsStatics.CANVAS_TOOLS_TITLE, "Yes", "No"))
                {
    ...
    private void SaveAssistantProperties()
    {
        ...
        AI_CodeAssistant.UpdateAICodeSettings(this.logicModel, this.sourceControl, this.temperature, this.maxTokens, this.topp, this.presencePenalty, this.frequencyPenalty);
    }
    ...
    private async void RefreshFineTunedModels(bool silent = true)
    {
        ...
        try
        {
            string result = await AI_CodeAssistant.GetSafeWebRequestAsync(AI_CodeAssistant.MODEL_ENDPOINT, this.openAIAPIKey);
    ...
    public static void UpdateAICodeSettings(OpenAILogicModel logicModel, OpenAISourceControl sourceControl, float temperature, int maxtokens, float topp, float ppenalty, float fpenalty)
    {
    ...
    private async void RemoveFineTunedModel()
    {
        ...
        try
        {
            await AI_CodeAssistant.DeleteWebRequestAsync(AI_CodeAssistant.MODEL_ENDPOINT, this.openAIAPIKey, this.fineTunedModel);
            RefreshFineTunedModels();
